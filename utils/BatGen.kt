...
class BdremBatGenerator(
    ...
    private fun buildBatText(
        ...
        val workersFallback = getTrackMuxValue(videoTrack, "workers", "8")
        val fastpassWorkers = getTrackMuxValue(videoTrack, "fastpassWorkers", workersFallback)
        val mainpassWorkers = getTrackMuxValue(videoTrack, "mainpassWorkers", workersFallback)
        val sceneDetectionRaw = getTrackMuxValue(videoTrack, "sceneDetection", "av1an").lowercase()
        val sceneDetection = if (sceneDetectionRaw == "psd" || sceneDetectionRaw == "av1an") {
            sceneDetectionRaw
        } else {
            "av1an"
        }
        ...
        val abPosMultiplier = getTrackMuxValue(videoTrack, "abPosMultiplier", "")
        val abNegMultiplier = getTrackMuxValue(videoTrack, "abNegMultiplier", "")
        val mainVpy = getTrackMuxValue(videoTrack, "mainVpy", "")
        val fastVpy = getTrackMuxValue(videoTrack, "fastVpy", "")
        val proxyVpy = getTrackMuxValue(videoTrack, "proxyVpy", "")
        ...
        val useAggressive = abMultiplier.isNotBlank()
        val usePosNeg = !useAggressive && abPosMultiplier.isNotBlank() && abNegMultiplier.isNotBlank()
        val usePsd = sceneDetection == "psd"
        val useFastVpy = fastVpy.isNotBlank()
        val useProxyVpy = proxyVpy.isNotBlank()
        val useMainpassVpyArgs = mainVpy.isNotBlank() || proxyVpy.isNotBlank()
        ...
        sb.appendLine("set \"AB_POS_MULT=$abPosMultiplier\"")
        sb.appendLine("set \"AB_NEG_MULT=$abNegMultiplier\"")
        sb.appendLine("set \"QUALITY=$crfValue\"")
        sb.appendLine()
        sb.appendLine("set \"FASTPASS_VF=$fastpassVf\"")
        sb.appendLine("set \"MAINPASS_VF=$mainpassVf\"")
        sb.appendLine("set \"FAST_VPY=$fastVpy\"")
        sb.appendLine("set \"MAIN_VPY=$mainVpy\"")
        sb.appendLine("set \"PROXY_VPY=$proxyVpy\"")
        sb.appendLine()
        sb.appendLine("REM ==========================================================")
        ...
        if (useFastVpy) sb.appendLine("  --fastpass-vpy \"%FAST_VPY%\" ^")
        if (useProxyVpy) sb.appendLine("  --fastpass-proxy \"%PROXY_VPY%\" ^")
        ...
        if (usePosNeg) {
            sb.appendLine("  --pos-dev-multiplier \"%AB_POS_MULT%\" ^")
            sb.appendLine("  --neg-dev-multiplier \"%AB_NEG_MULT%\" ^")
        }
        if (abPosDev.isNotBlank()) sb.appendLine("  --max-positive-dev \"%AB_POS_DEV%\" ^")
        if (abNegDev.isNotBlank()) sb.appendLine("  --max-negative-dev \"%AB_NEG_DEV%\" ^")
        ...
        sb.appendLine("${Paths.AV1AN_EXE} -i \"%MAINPASS_INPUT%\" -o \"%VIDEO_OUT%\" ^") 
        sb.appendLine("  --scenes \"%SCENES_FINAL%\" ^")
        sb.appendLine("  --workers \"%MAINPASS_WORKERS%\" ^")
        sb.appendLine("  --temp \"%VIDEO_TEMP%\\mainpass\" ^")
        if (useProxyVpy) sb.appendLine("  --proxy \"%PROXY_VPY%\" ^")
        if (useMainpassVpyArgs) sb.appendLine("  --vspipe-args \"src=%SRC%\" ^")
        sb.appendLine("  -n ^")
        sb.appendLine("  --keep ^")
        sb.appendLine("  --verbose ^")
        ...
        // Mux stage
        sb.appendLine("REM ==========================================================")
        sb.appendLine("REM  5) MUX -> output *-av1.mkv")
        sb.appendLine("REM ==========================================================")
        sb.appendLine()
        sb.appendLine("${Paths.VS_PYTHON_EXE} ${q(muxPy)} --no-source-bitrate --source \"%SRC%\" --workdir \"%WORKDIR%\" --log \"%LOGDIR%\\09_mux.log\"")
        sb.appendLine("if errorlevel 1 goto :fail")
        sb.appendLine()
