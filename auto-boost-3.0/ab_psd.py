...
def run_psd(
    ...
    for arg in [str(item) for item in (vspipe_args or []) if str(item).strip()]:
        cmd.extend(["--vspipe-arg", arg])

    source_for_events = event_source or input_file
