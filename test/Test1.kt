...
fun main() {
    summarizeTracksWithPython(
        "python",
        "C:\\systemUtils\\file-tracks-info.py",
        Files.list(Path.of("C:\\Users\\prio7\\Downloads\\Gachiakuta TV-1 [anidb2-18686]")).toList().filter { it.extension == "mkv" }.map { it.pathString }
    ).forEach { println(it) }
...
//    buildSummary(list1.groupBy { it.fileIndex }.size,list1).forEach { println(it) }
}


private fun summarizeTracksWithPython(
    pythonExe: String,
    scriptPath: String,
    files: List<String>
): List<String> {
...
private fun parseTracksFromScriptOutput(
    output: String,
    filesCount: Int
): List<RawTrack> {
...
private fun formatFileSet(files: Set<Int>): String {
...
private fun buildSummary(
    ...
    data class TrackKey(val type: String, val ident: String)

    // Build trackKey -> (fileIndex -> ffIndex)
    val trackMap = mutableMapOf<TrackKey, MutableMap<Int, Int>>()
    for (t in tracks) {
        val key = TrackKey(t.type, identFor(t))
    ...
    for ((idx, atIndex) in tracksByIndex) {
        ...
        val groups = mutableMapOf<Pair<String, String>, MutableSet<Int>>()
        for (t in atIndex) {
            val displayName = identFor(t)
            val key = t.type to displayName
            groups.getOrPut(key) { mutableSetOf() } += t.fileIndex
        }

        for ((key, filesSet) in groups) {
            val (type, displayName) = key
            rows += SummaryRow(
                index = idx,
                star = star,
                type = type,
    ...
    rowsByIndex.toSortedMap().forEach { (_, idxRows) ->
        ...
        for (row in sortedRows) {
            ...
            val base = "$starPrefix${row.index} | ${row.type} | ${row.displayName}"
